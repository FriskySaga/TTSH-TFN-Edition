...
import RaceGlobals, random, os, json
...
class RaceManagerAI(DirectObject.DirectObject):
    ...
    def checkForRaceTrophies(self, race, avId, inHistory=None):
        ...
        for genre in xrange(numTrackGenres):
    ...
    def checkForNonRaceTrophies(self, avId, inHistory=None):
        ...
        for i in xrange(1, RaceGlobals.NumTrophies / RaceGlobals.TrophiesPerCup + 1):
    ...
    def checkHistoryForTrophyByValue(self, trophies, history, historyIndexValue, trophyReqList, trophyIndices):
        ...
        for index in xrange(len(trophyIndices)):
    ...
    def mergeHistories(self, historyA, historyB):
        ...
        mergedHistory = []
        for index in xrange(newHistorySize):
    ...
    def checkForTrophy(self, place, trackId, raceType, numRacers, avId, historyIndex, trophyIndices, trophyReqList, historyTotalList, totalTrophyIndex, totalReq):
        ...
        if history[historyIndex] < trophyReqList[-1]:
            history[historyIndex] += 1
            newHistory = 1
        for i in xrange(0, len(trophyReqList)):
    ...
    def checkTimeRecord(self, trackId, time, raceType, numRacers, avId):
        ...
        for period in RaceGlobals.PeriodIds:
            for record in xrange(0, RaceGlobals.NumRecordsPerPeriod):
                ...
                if time < recordTime:
                    ...
                    if av:
                        name = av.name
                        self.trackRecords[str(trackId)][str(period)].insert(record, (time, raceType, numRacers, name))
                        self.trackRecords[str(trackId)][str(period)] = self.trackRecords[str(trackId)][str(period)][:RaceGlobals.NumRecordsPerPeriod]
        ...
        for trackId in RaceGlobals.TrackIds:
            if str(trackId) not in records:
                ...
                for i in RaceGlobals.PeriodIds:
                    ...
                    for j in xrange(0, RaceGlobals.NumRecordsPerPeriod):
                        records[str(trackId)][str(i)].append(RaceGlobals.getDefaultRecord(trackId))
    ...
    def resetRecordPeriod(self, period):
        for trackId in RaceGlobals.TrackIds:
            for i in xrange(0, RaceGlobals.NumRecordsPerPeriod):
                self.trackRecords[str(trackId)][str(period)][i] = RaceGlobals.getDefaultRecord(trackId)
